/* ============================================
   ...
   FIXED VERSION v3 — Cache Buster + Updated Fallback
   ============================================ */

// ============================================
// EMBEDDED FALLBACK DATA (updated to match today.md)
// ============================================
...
// ... (sisa kode sama, hanya ubah loadTodayDevotion seperti di bawah)
...
async function loadTodayDevotion() {
    ...
    // CACHE BUSTER: tambahkan timestamp agar selalu ambil file terbaru
    const cacheBuster = '?v=' + Date.now();

    for (const source of sources) {
        try {
            const url = source + (source.includes('?') ? '&' : '') + cacheBuster;
            console.log('Trying to load from:', url);
            const response = await fetch(url);
            
            if (response.ok) {
                const markdown = await response.text();
                console.log('Loaded from:', source, 'Length:', markdown.length);
                ...
                console.log('Response not OK:', source, response.status);
            }
        } catch (e) {
    ...
    console.log('All sources failed. Using embedded fallback data');
